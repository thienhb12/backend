<?php
class sendinfoBackEnd extends Anpro_Module_Base 
{
	...
	{
		$id = $_GET["id"];
		$this -> deleteImage($aItems[$i], "file1", $this -> img_path);
		$this -> deleteImage($aItems[$i], "file2", $this -> img_path);
		$this -> deleteImage($aItems[$i], "file3", $this -> img_path);
	...
	function buildForm( $data=array() ,$msg = ''){
		...
		if($data['file1'])
			$img_group[] = $form->createElement('static',null,'',"<a href='".SITE_URL.$data['file1']."' target='_blank' ><img src='".SITE_URL.$data['file1']."' width=100 border=0></a>");
		if($data['file2'])
			$img_group[] = $form->createElement('static',null,'',"<a href='".SITE_URL.$data['file2']."' target='_blank' ><img src='".SITE_URL.$data['file2']."' width=100 border=0></a>");
		if($data['file3'])
			$img_group[] =  $form->createElement('static',null,'',"<a href='".SITE_URL.$data['file3']."' target='_blank' ><img src='".SITE_URL.$data['file3']."' width=100 border=0></a>");
