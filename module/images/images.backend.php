...
class imagesBackEnd extends Anpro_Module_Base 
{
	...
	function manageImage()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST')
		{
			...
			if($_FILES['avatar']['name']!='')
			{
				...
				$uploadOk = 1;
				if(!$this -> checkExtensionFileUpload($extension, strtolower($file_name))) {
				...
				if($uploadOk) {
					...
					$aData['name'] = $this -> renameFile($file_name);
                                        $aData['site_id'] = $this -> site_id;
                                        $aData['user_id'] = $_SESSION['userid'];
		...
		$where .= " AND site_id = ". $_SESSION['site_id'];
        ...
        function getListImage()
	{
		$to_date = date('d-m-Y') ;
		$from_date = date('d-m-Y', mktime(0, 0, 0, date("m")-1, date("d"),   date("Y")));
		$where = " url != '' and site_id ={$this ->site_id} ";
