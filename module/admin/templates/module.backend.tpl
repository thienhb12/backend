...
<form class="form-horizontal" method="post" action="{$action_url}" role="form" data-toggle="validator">
		...
		<label for="inputProvince" class="col-sm-2 control-label">{#group#}</label>
		 <div class="col-sm-5">
			<select class="form-control" id="parent_id" name="parent_id" >
				...
				{html_options options=$modules_list selected=$detail.parent_id}
			</select>
		</div>
	</div>
    <div class="form-group">
  ...
  <div class="form-group">
	...
	<div class="col-sm-5">
		 <input type="text" class="form-control" id="link" required  placeholder="" name="link" value="{$detail.link}">
		 <span class="help-block with-errors"></span>
	</div>
  </div>
  <div class="form-group">
    <div class="col-sm-offset-2 col-sm-10">
	  <ul class="list-group">
		{foreach $roll_list as $roll}
			<li>
				<input type="checkbox" name="module_roll[]" value="{$roll.id}" {if $roll.checked}checked{/if}> {$roll.name} ({$roll.task})
			</li>
		{/foreach}
	  </ul>
    </div>
