...
class tintuc extends Anpro_Module_Base 
{
	...
	function getTinMoi()
	{
		$template = 'tinmoi_tintuc.tpl';
		if(1) {
			$where = " status = 1 AND type = '{$this-> type}'";
			$items = $this -> getListTitle($where, 10,  'id DESC');
			$this -> smarty -> assign('items',$items);

			$cond = " status = 1 AND  DATEDIFF( DATE( create_date ) , NOW( ) ) > -2";

			$mostread = $this -> getListTitle($cond, 10, " count DESC");
